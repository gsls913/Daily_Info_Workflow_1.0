...
切换模型方案:
在config/ai_models.json中修改ai_provider为"modelscope"或"huoshan"。
...
from common_libs.utils.paths import PROJECT_ROOT, CREDENTIALS_DIR, TOKEN_FILE, ALPHAPAI_INFO_FILE, MEMO_BASE_DIR, MEMO_HISTORY_FILE
...
def auto_download_all_tags(max_count_per_tag=None, auto_tag=True):
    ...
    for tag_id in range(1, 9):
        ...
        print(f"\n{'=' * 70}")
        print(f"🏷️  [{tag_id}/8] {tag_config['name']}")
        print(f"{'=' * 70}")
    ...
    if auto_tag and all_downloaded_files:
        ...
        try:
            from common_libs.ai.ai_client import get_current_provider
            current_provider = get_current_provider()
            provider_display = {"huoshan": "火山引擎", "minimax": "MiniMax"}.get(current_provider, "ModelScope")
            print(f"🤖 当前AI模式: {provider_display} ({current_provider})")
            print(f"{'=' * 70}\n")
            
            from common_libs.ai.aicontent_generator import generate_tags_and_analysis_for_batch_parallel
...
def save_markdown(meeting, detail, output_dir, api_type="standard"):
    ...
    if detail:
        parts += ["# 基本信息", ""]
        parts.append(f"- **日期**: {detail.get('roadshowDate', date_raw)}")
        parts.append(f"- **机构**: {detail.get('publishInstitution', meeting.get('institutionName', '未知'))}")
        parts.append(f"- **分析师**: {detail.get('analyst', meeting.get('analystName', '未知'))}")
        parts.append(f"- **嘉宾**: {detail.get('guest', meeting.get('guestNames', '无'))}")
