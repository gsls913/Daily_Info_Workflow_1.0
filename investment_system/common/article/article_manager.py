...
def _unique_destination_path(dest_path):
    if not os.path.exists(dest_path):
        return dest_path
    folder = os.path.dirname(dest_path)
    stem, suffix = os.path.splitext(os.path.basename(dest_path))
    for idx in range(1, 10000):
        candidate = os.path.join(folder, f"{stem}_{idx}{suffix}")
    ...
    for md_file in md_files:
        ...
        if check_if_read(md_file_path):
            ...
            try:
                ...
                total_archived += 1
                _safe_print(f"  ✓ 已归档: {md_file} → 已读/{folder_name}/{os.path.basename(dest_path)}")
            except Exception as e:
...
def clean_old_read_articles(base_dir, subfolders, days_threshold=90, attachment_dir=None, date_field="日期"):
    ...
    for subfolder in subfolders:
        ...
        for md_file in md_files:
            ...
            if days_diff > days_threshold:
                ...
                try:
                    move_to_recycle_bin(md_file_path, category="read_articles", item_type="markdown")
                    deleted_articles += 1
                    _safe_print(f"  ✓ 已移入回收站: {md_file} ({days_diff}天前)")
                except Exception as e:
                    _safe_print(f"移动文件到回收站失败 {md_file}: {e}")
                    continue
                ...
                if attachment_dir:
                    for image_name in images:
                        ...
                        if _image_referenced_elsewhere(base_dir, image_name, md_file_path):
                            ...
                            continue
                        try:
                            move_to_recycle_bin(image_path, category="read_articles", item_type="image")
                            deleted_images += 1
                            _safe_print(f"  ✓ 图片已移入回收站: {image_name}")
                        except Exception as e:
                            _safe_print(f"图片移动到回收站失败 {image_name}: {e}")
    ...
    if total_deleted_articles > 0:
        ...
        _safe_print(f"📊 清理统计: 总计移入回收站 {total_deleted_articles} 篇过期文章{img_msg}")
