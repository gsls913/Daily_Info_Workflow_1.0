...
def _load_config():
    global _config_cache, _config_load_error
    ...
    try:
        ...
        _config_load_error = f"config.yaml 解析失败: {_CONFIG_FILE} ({e})"
        print(f"Error: {_config_load_error}")
        _config_cache = {}
...
def get(key_path, default=None):
    ...
    if _config_load_error:
        raise RuntimeError(_config_load_error)
...
def get_memo_base_dir():
    ...
    subdir = get('paths.memo_base_dir', 'C会议纪要')
    return os.path.join(base, subdir)


def get_wechat_article_base_dir():
    base = get_obsidian_base_dir()
    subdir = get('paths.wechat_article_base_dir', 'C微信文章')
    return os.path.join(base, subdir)
