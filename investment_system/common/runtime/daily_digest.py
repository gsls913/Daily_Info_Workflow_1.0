...
from investment_system.common.ai.quality import basic_ai_response_ok, clean_thinking_tags
...
from investment_system.common.runtime.task_state import load_task_state
from investment_system.common.storage.download_history import load_json_with_backup, save_json_atomic
from investment_system.common.utils.paths import OBSIDIAN_BASE_DIR, PROJECT_ROOT
...
DAILY_DIGEST_PROMPT = """你是一位个人认知助理和信息总编。下面是今天自动收集到的会议纪要、微信文章和播客要点正文。

请写一份“Daily Brief / 信息汇总日报”，目标不是复述材料，而是帮助我每天更好地获取、理解、消化、保存重要信息，逐步形成更好的认知状态。

请严格使用以下 Markdown 结构：

## 今日最重要的 5-10 个信号
- 每条用 1-3 句话说明：发生了什么、为什么重要、可能影响什么。
- 不要平均用力，优先选择认知增量高、长期价值高、反常识或影响较大的信息。

## 主题脉络
- 按主题组织内容，不按来源流水账。
- 每个主题下区分“事实/信息”“观点/解释”“我的综合判断”。

## 值得进入长期记忆的概念
- 提炼今天值得保存到 Obsidian 的概念、框架、判断或案例。
- 如果材料里没有足够内容，可以少写，不要硬凑。

## 风险、分歧与不确定性
- 明确哪些内容仍需验证，哪些观点之间存在冲突，哪些信号可能只是噪音。

## 明天或未来值得留意
- 给出少量值得继续观察的线索，不要变成泛泛的行动清单。

写作要求：
1. 明确区分事实、观点和你的综合判断；不要编造原文没有的信息。
2. 对明显重复的信息做合并，对信息密度低的内容可以简略处理。
3. 输出 Markdown，结构清晰，适合保存到 Obsidian。
...
CHUNK_PROMPT = """你是一位个人认知助理和信息筛选助手。下面是今日材料的一部分。

请提炼这一部分中值得进入 Daily Brief 的内容：
- 重要事实、数据、公司、行业、政策、技术、商业、社会变化或风险信号
- 有启发、反常识、能改变理解框架的观点
- 值得进入长期记忆的概念、案例、判断
- 仍需验证的分歧、不确定性和后续线索
- 可以忽略的信息请压缩或省略
...
SYNTHESIS_PROMPT = """你是一位个人认知助理和信息总编。下面是今日材料分段提炼后的结果。

请综合成一份完整的“Daily Brief / 信息汇总日报”，用于帮助我形成更好的每日认知状态。
...
要求：
1. 按主题组织，不按分段流水账。
2. 合并重复内容，保留认知增量高、长期价值高、反常识或影响较大的信息。
3. 明确区分事实、观点和综合判断。
4. 输出 Markdown，适合保存到 Obsidian。
...
WEEKLY_DIGEST_PROMPT = """你是一位个人认知助理和周度信息总编。下面是本周每天的信息汇总日报。

请写一份“Weekly Review / 信息汇总周报”，目标是帮助我从一周的信息流里看见趋势、变化、重复出现的线索、认知增量和长期应保存的判断。
...
写作要求：
1. 强调变化、共识、分歧和反复出现的线索。
2. 按主题组织，不要按日期流水账复述。
3. 明确区分事实、观点和综合判断；不要编造日报里没有的信息。
4. 输出 Markdown，结构清晰，适合保存到 Obsidian。
...
WEEKLY_CHUNK_PROMPT = """你是一位个人认知助理和周度信息筛选助手。下面是本周日报材料的一部分。

请提炼这一部分中值得进入周报的内容：
- 本周反复出现或互相印证的重要线索
- 重要事实、公司、行业、政策、技术、商业、社会变化或风险信号
- 认知增量、反常识观点、长期应保存的概念或判断
- 下周值得继续观察的问题
- 可以忽略的日常噪音请压缩或省略
...
WEEKLY_SYNTHESIS_PROMPT = """你是一位个人认知助理和周度信息总编。下面是本周日报材料分段提炼后的结果。

请综合成一份完整的“Weekly Review / 信息汇总周报”。
...
def _load_state() -> dict[str, Any]:
    state = load_json_with_backup(STATE_FILE, default={})
...
def claim_first_full_run_today() -> bool:
    """在启动器开始时判定今天是否仍需要尝试生成日报。"""
    today = _today()
    state = _load_state()
    already_generated = (
        state.get("digest_generated_date") == today
        or (
            state.get("first_full_run_date") == today
            and state.get("digest_status") == "generated"
        )
    )
    if already_generated:
        return False
    state["candidate_full_run_date"] = today
    state["candidate_full_run_at"] = datetime.now().isoformat(timespec="seconds")
...
def claim_first_full_run_this_week() -> bool:
    """在启动器开始时判定本周是否仍需要尝试生成周报。"""
    week_key = _week_start().isoformat()
    state = _load_state()
    already_generated = (
        state.get("weekly_digest_generated_week") == week_key
        or (
            state.get("first_full_run_week") == week_key
            and state.get("weekly_digest_status") == "generated"
        )
    )
    if already_generated:
        return False
    state["candidate_full_run_week"] = week_key
    state["candidate_full_run_week_at"] = datetime.now().isoformat(timespec="seconds")
...
def mark_digest_result(status: str, path: str = "", error: str = "") -> None:
    ...
    if status == "generated":
        state["first_full_run_date"] = _today()
        state["first_full_run_at"] = state.get("candidate_full_run_at") or state["digest_finished_at"]
        state["digest_generated_date"] = _today()
    if path:
...
def mark_weekly_digest_result(status: str, path: str = "", error: str = "") -> None:
    ...
    if status == "generated":
        ...
        state["first_full_run_week_at"] = state.get("candidate_full_run_week_at") or state["weekly_digest_finished_at"]
        state["weekly_digest_generated_week"] = week_key
    if path:
...
def _daily_dirs() -> tuple[Path, Path]:
    base = Path(OBSIDIAN_BASE_DIR) / cfg("daily_digest.base_dir", "Cz日报周报")
...
def generate_daily_digest(log_func: LogFunc = print) -> Path | None:
    ...
    try:
        ...
        source_path.write_text(normalize_markdown_output(source_text), encoding="utf-8")
