...
from .quality import basic_ai_response_ok, clean_thinking_tags
...
def load_tag_options(excel_path: Optional[Path] = None) -> Dict[str, List[str]]:
    ...
    """
    from investment_system.common.config.source_config import load_tag_options as _load_tag_options

    return _load_tag_options(excel_path)
...
def update_md_tags(md_file_path: Path, industry_tags: List[str], company_tags: List[str]) -> bool:
    ...
    try:
        ...
        # 写回文件
        md_file_path.write_text(normalize_markdown_output(new_content), encoding="utf-8")
        return True
...
def generate_ai_analysis(
    ...
    try:
        ...
        ok, reason = basic_ai_response_ok(response, min_chars=80)
        if not ok:
