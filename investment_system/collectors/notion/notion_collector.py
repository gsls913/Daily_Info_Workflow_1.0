...
from investment_system.common.utils.paths import PROJECT_ROOT, WECHAT_ARTICLE_BASE_DIR, ATTACHMENT_DIR, NOTION_CONFIG_FILE, NOTION_WECHAT_HISTORY_FILE, NOTION_WECHAT_FAILED_ARTICLES_FILE
...
from investment_system.common.wechat_downloader.wechat_to_md import download_wechat_article, is_sticker_article
...
FAILED_ARTICLES_FILE = NOTION_WECHAT_FAILED_ARTICLES_FILE
...
PROPERTY_TITLE = cfg('notion.property_title', '标题')
NOTION_PAGE_SIZE = cfg('notion.page_size', 100)
...
def mark_downloaded_with_recovery(
    notion,
    page_id,
    url,
    local_history,
    ...
    if not mark_as_downloaded(notion, page_id, title=title, title_property_type=title_property_type):
...
def get_property_type(properties: dict, property_name: str) -> str:
    prop = (properties or {}).get(property_name, {})
    return prop.get("type", "")


def build_notion_text_property(property_name: str, property_type: str, text: str) -> dict:
    """根据 Notion 字段类型构造文本属性更新 payload。"""
    text = (text or "").strip()
    if not property_name or not text:
        return {}

    text_value = [{"text": {"content": text[:2000]}}]
    if property_type == "title":
        return {property_name: {"title": text_value}}
    if property_type == "rich_text":
        return {property_name: {"rich_text": text_value}}
    return {}


def update_page_properties_with_retry(notion: Client, page_id: str, properties: dict, action_label: str) -> bool:
    """更新 Notion 页面属性（带重试机制）"""
    max_retries = 3
    retry_delay = 2
...
def mark_as_downloaded(
    ...
    page_id: str,
    title: str = None,
    title_property_type: str = None
) -> bool:
    """标记文章为已下载，并在可用时写入文章标题。

    Args:
        notion: Notion客户端
        page_id: 页面ID
        title: 微信文章标题
        title_property_type: Notion 标题列类型，支持 title/rich_text
    ...
    title_property = build_notion_text_property(PROPERTY_TITLE, title_property_type, title)
    if title and not title_property:
        log_warn(f"  ⚠️ Notion 字段「{PROPERTY_TITLE}」不是可写文本/标题类型，跳过标题写入")

    properties = {**downloaded_property, **title_property}
    if update_page_properties_with_retry(notion, page_id, properties, "更新状态"):
        return True

    if title_property:
        log_warn("  ⚠️ 标题与状态合并更新失败，回退为只更新下载状态")
        return update_page_properties_with_retry(notion, page_id, downloaded_property, "更新状态")

    return False


def update_article_title(notion: Client, page_id: str, title: str, title_property_type: str = None) -> bool:
    """单独写入文章标题，不改变下载状态。"""
    title_property = build_notion_text_property(PROPERTY_TITLE, title_property_type, title)
    if not title_property:
        return False
    return update_page_properties_with_retry(notion, page_id, title_property, "更新标题")
...
def main():
    ...
    try:
        ...
        title_property_type = get_property_type(db_info.get("properties", {}), PROPERTY_TITLE)
        if title_property_type in ("title", "rich_text"):
            log_info(f"   标题字段: {PROPERTY_TITLE} ({title_property_type})")
        else:
            log_warn(f"   未找到可写标题字段「{PROPERTY_TITLE}」，本次只更新下载状态")
    except Exception as e:
        log_error(f"连接失败: {e}")
        return 1
    ...
    for i, page in article_iterator:
        ...
        if not url:
            ...
            mark_downloaded_with_recovery(
                notion,
                page_id,
                url,
                processed_history,
                status="skipped_no_url",
                title_property_type=title_property_type
            )
        ...
        if is_processed_locally(processed_history, page_id, url):
            ...
            download_stats.add_skipped()
            processed_record = get_processed_record(processed_history, page_id, url)
            mark_as_downloaded(
                notion,
                page_id,
        ...
        try:
            ...
            if result.get("success"):
                ...
                if "贴图文章" in error:
                    ...
                    update_article_title(notion, page_id, result.get("title"), title_property_type)
