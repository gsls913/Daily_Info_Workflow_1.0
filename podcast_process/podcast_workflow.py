...
from common_libs.config.config_loader import get as cfg, get_podcast_accounts
from common_libs.ai.quality import basic_ai_response_ok
from common_libs.runtime.task_state import update_task
from common_libs.storage.download_history import load_download_history, save_json_atomic
from common_libs.utils.logging_config import clean_old_logs, get_log_functions, setup_logging
from common_libs.utils.notifications import show_windows_notification
from common_libs.utils.paths import PODCAST_BASE_DIR, PODCAST_HISTORY_FILE, PROJECT_ROOT as ROOT_FROM_PATHS, CREDENTIALS_DIR
from podcast_process.xiaoyuzhou_parser import Episode, Podcast, fetch_podcast_info

from tingwu_api_upload import find_ffprobe, upload_one, validate_media_file
from tingwu_common import DEFAULT_STORAGE_STATE, load_cookie, request_headers
from tingwu_delete_record import DEFAULT_PAGE_SIZE as TINGWU_LIST_DEFAULT_PAGE_SIZE
from tingwu_delete_record import delete_trans, find_trans_by_id, get_trans_status as get_delete_trans_status
from tingwu_export_download import download_file, export_trans, get_export_url, get_trans_status
...
def clean_old_read_notes(days_threshold: int = READ_RETENTION_DAYS) -> int:
    ...
    for md_file in PODCAST_READ_DIR.glob("*.md"):
        ...
        if (now - pub_date).days > days_threshold:
            md_file.unlink()
            deleted += 1
            log_info(f"已删除过期播客笔记: {md_file.name}")
    return deleted
...
def select_new_episodes(podcast: Podcast, history: dict[str, Any]) -> list[Episode]:
...
def process_completed_transcripts(cookie: str, history: dict[str, Any], max_pages: int | None = None) -> int:
...
def upload_new_podcast_episodes(cookie: str, history: dict[str, Any], limit_accounts: int | None = None) -> int:
    accounts = [normalize_account(item) for item in get_podcast_accounts()]
    accounts = [item for item in accounts if item.get("url")]
    if limit_accounts:
        accounts = accounts[:limit_accounts]
    if not accounts:
        log_warn("未配置小宇宙播客账号，请在 config/config.yaml 的 podcast.accounts 中添加")
        return 0
    ...
    for idx, account in enumerate(accounts, 1):
        ...
        if not podcast:
            ...
            continue
        episodes = select_new_episodes(podcast, history)
...
def main() -> None:
    ...
    parser.add_argument("--phase", choices=["all", "archive", "process-completed", "upload-new"], default="all")
    ...
    if args.phase in ("all", "process-completed", "upload-new"):
        try:
            cookie = ensure_tingwu_cookie(args.storage_state, refresh_login=REFRESH_TINGWU_LOGIN_EACH_RUN)
        except Exception as e:
            log_error(f"通义听悟登录状态不可用: {e}")
            log_error("自动登录未能恢复。可手动运行: python podcast_process\\tingwu_python_workflow\\tingwu_profile.py auto-login --headed --wait-verification-seconds=60")
            raise SystemExit(1)
        ...
        if args.phase in ("all", "process-completed"):
            ...
            processed = process_completed_transcripts(cookie, history, max_pages=max_completed_pages)

        if args.phase in ("all", "upload-new"):
            uploaded = upload_new_podcast_episodes(cookie, history, limit_accounts=args.limit_accounts)
