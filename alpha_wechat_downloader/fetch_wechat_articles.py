...
from common_libs.utils.paths import PROJECT_ROOT, CREDENTIALS_DIR, TOKEN_FILE, ALPHAPAI_INFO_FILE, WECHAT_ARTICLE_BASE_DIR, ATTACHMENT_DIR, OBSIDIAN_BASE_DIR, WECHAT_HISTORY_FILE, SET_CONFIG_FILE
...
def load_download_history():
    from common_libs.storage.download_history import load_download_history as _load
    return _load(HISTORY_FILE)


def save_download_history(history):
    from common_libs.storage.download_history import save_download_history as _save
...
def load_wechat_accounts_from_excel():
    ...
    if not os.path.exists(CONFIG_FILE):
        raise FileNotFoundError(
            ...
            f"  A列: 公众号名称\n"
            f"  B列: 简称\n"
            f"  C列: 分类(投资/宏观/商业/科技/工作/其他)"
        )
    
    try:
        wb = load_workbook(CONFIG_FILE)
        
        if 'wechat_account' not in wb.sheetnames:
            raise ValueError(
                f"Excel文件中缺少 'wechat_account' sheet\n"
                f"当前可用的sheets: {wb.sheetnames}\n"
                f"请创建名为 'wechat_account' 的sheet"
            )
        
        ws = wb['wechat_account']
        
        # 验证数据
        if ws.max_row < 2:
            raise ValueError(
                "wechat_account sheet 中没有数据\n"
                "第一行应为列标题，第二行开始为数据"
            )
        
        accounts = []
        for row_idx, row in enumerate(ws.iter_rows(min_row=2, values_only=True), start=2):
            if not row[0]:  # 公众号名称为空
                continue
            
            name = str(row[0]).strip()
            short_name = str(row[1]).strip() if row[1] else name[:2]
            category = str(row[2]).strip() if row[2] else "其他"
            
            # 验证分类
            if category not in CATEGORY_DIR_MAP:
                log_warn(f"⚠️  行{row_idx}: 无效分类 '{category}'，使用默认分类 '其他'")
                category = "其他"
...
def fetch_articles_until_downloaded(headers, account_info, history, max_pages=10):
    """
    获取公众号文章，直到找到已下载的为止
    - 新公众号（无历史记录）：下载5篇
    - 有记录的公众号：下载未下载过的文章
    ...
    max_articles = NEW_ACCOUNT_DOWNLOAD_COUNT if is_new_account else MAX_DOWNLOAD_PER_ACCOUNT
...
def save_article_from_api(article: dict, short_name: str, category: str) -> dict:
...
def fetch_and_download_articles(headers, account_info, history, account_index=0, total_accounts=1):
    """获取并下载公众号文章
    ...
    articles = fetch_articles_until_downloaded(headers, account_info, history)
    ...
    for idx, article in article_iterator:
        ...
        result = save_article_from_api(article, short_name, category)
    ...
    for idx, account_info in enumerate(matched_accounts):
        try:
            count = fetch_and_download_articles(
                ...
                account_index=idx, total_accounts=download_stats.total_accounts
            )
        except Exception as e:
