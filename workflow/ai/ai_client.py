...
COMBINE_DIR = SCRIPT_DIR.parent
PROJECT_ROOT = COMBINE_DIR.parent
CONFIG_DIR = SCRIPT_DIR
AI_MODELS_CONFIG_FILE = CONFIG_DIR / "ai_models.json"
...
def _load_api_keys() -> Dict[str, str]:
    ...
    # 先从环境变量读取
    for provider in ["huoshan", "modelscope", "minimax"]:
...
def _get_api_key(provider: str) -> str:
    ...
    Args:
        provider: "modelscope", "huoshan" 或 "minimax"
...
class AIClient:
    ...
    支持三种AI提供商：modelscope、huoshan 和 minimax
    ...
    def __init__(self, log_func: Optional[Callable[[str, str], None]] = None, provider: Optional[str] = None):
        ...
        Args:
            ...
            provider: AI提供商，"modelscope"、"huoshan" 或 "minimax"，默认从配置文件读取
        """
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if AI_MODELS_CONFIG_FILE.exists():
                        AI_MODELS_CONFIG_FILE.unlink()
                    os.rename(temp_file, AI_MODELS_CONFIG_FILE)
                    return
    ...
    def _call_api(
        ...
        """调用 API（根据提供商选择）"""
        if self._provider == "huoshan":
            return self._call_api_huoshan(model, prompt, temperature, max_tokens)
        elif self._provider == "minimax":
            return self._call_api_minimax(model, prompt, temperature, max_tokens)
        else:
            return self._call_api_modelscope(model, prompt, temperature, max_tokens)
    ...
    def call(
        ...
        # 火山引擎和MiniMax：使用单模型重试逻辑
        if self._provider in ["huoshan", "minimax"]:
    ...
    def _call_single_model_logic(
        ...
        """单模型调用逻辑（火山引擎和MiniMax使用）"""
        api_config = self._get_api_config()
        retry_delay = api_config.get("retry_delay_seconds", 10)
        
        model = HUOSHAN_MODEL if self._provider == "huoshan" else self._get_minimax_model()
        attempts = 0
        last_error = None
            ...
            try:
                ...
                if "速率限制" in error_msg or "rate limit" in error_msg.lower():
                    ...
                    if attempts < max_attempts_per_model:
                        self._log(f"[{model}] 速率限制，等待{retry_delay}秒后重试...", "WARN")
                        time.sleep(retry_delay)
...
def get_parallel_workers(provider: Optional[str] = None) -> int:
    ...
    Returns:
        并行工作线程数，modelscope默认5，huoshan默认20，minimax默认10
    """
    try:
        if AI_MODELS_CONFIG_FILE.exists():
            with open(AI_MODELS_CONFIG_FILE, "r", encoding="utf-8") as f:
            ...
            if provider == "huoshan":
                ...
                return config.get("minimax_parallel_workers", 10)
            else:
